...
/*
This file was automatically generated from '../../contrib/yolog/srcutil/genyolog.pl'.
...
struct epoxy_yolog_implicit_st {
    int level;
    ...
    struct epoxy_yolog_context *ctx;
};
...
#define epoxy_yolog_global_init() pthread_mutex_init(&Yolog_Global_Mutex, NULL)
#define epoxy_yolog_global_lock() pthread_mutex_lock(&Yolog_Global_Mutex)
#define epoxy_yolog_global_unlock() pthread_mutex_unlock(&Yolog_Global_Mutex)
#define epoxy_yolog_dest_lock(ctx) flockfile(ctx->fp)
#define epoxy_yolog_dest_unlock(ctx) funlockfile(ctx->fp)

#else
#define epoxy_yolog_dest_lock(ctx)
#define epoxy_yolog_dest_unlock(ctx)
#define epoxy_yolog_global_init()
#define epoxy_yolog_global_lock()
#define epoxy_yolog_global_unlock()
#endif /* __unix __ */

#include "epoxy_yolog.h"

static struct epoxy_yolog_implicit_st Yolog_Implicit;
static
epoxy_yolog_context Yolog_Global_Context = {
        EPOXY_YOLOG_LEVEL_UNSET, /* level */
        NULL, /* parent */
        { 0 }, /* level settings */
        "", /* prefix */
};
...
static
epoxy_yolog_context_group Yolog_Global_CtxGroup = {
...
EPOXY_YOLOG_API
void
epoxy_yolog_init_defaults(struct epoxy_yolog_context_group *grp,
                    ...
                    const char *level_env)
{
    ...
    if (level_env && (envtmp = getenv(level_env))) {
        sscanf(envtmp, "%d", &itmp);
        if (itmp != EPOXY_YOLOG_DEFAULT) {
            default_level = itmp;
        }
    }

    if (!grp->o_screen.fmtv) {
        grp->o_screen.fmtv = epoxy_yolog_fmt_compile(EPOXY_YOLOG_FORMAT_DEFAULT);
    }
    ...
    if (default_level == EPOXY_YOLOG_LEVEL_UNSET) {
        default_level = EPOXY_YOLOG_INFO;
    }

    grp->o_screen.level = default_level;

    for (ii = 0; ii < grp->ncontexts; ii++) {
        epoxy_yolog_context *ctx = grp->contexts + ii;
        ctx->parent = grp;
        ...
        epoxy_yolog_sync_levels(ctx);
    }

    if (grp != &Yolog_Global_CtxGroup) {
        epoxy_yolog_init_defaults(&Yolog_Global_CtxGroup,
                            default_level,
                            color_env,
                            level_env);
    } else {
        epoxy_yolog_global_init();
    }
}

static int
output_can_log(epoxy_yolog_context *ctx,
            int level,
            int oix,
            struct epoxy_yolog_output_st *output)
{
    if (output == NULL) {
        return 0;
    }
    ...
    if (ctx->olevels[oix] != EPOXY_YOLOG_LEVEL_UNSET) {
    ...
    if (output->level != EPOXY_YOLOG_LEVEL_UNSET) {
...
static int
ctx_can_log(epoxy_yolog_context *ctx,
            int level,
            struct epoxy_yolog_output_st *outputs[EPOXY_YOLOG_OUTPUT_COUNT+1])
{
    int ii;
    outputs[EPOXY_YOLOG_OUTPUT_GFILE] = &ctx->parent->o_file;
    outputs[EPOXY_YOLOG_OUTPUT_SCREEN] = &ctx->parent->o_screen;
    outputs[EPOXY_YOLOG_OUTPUT_PFILE] = ctx->o_alt;

    if (ctx->level != EPOXY_YOLOG_LEVEL_UNSET && ctx->level > ctx->level) {
        return 0;
    }

    for (ii = 0; ii < EPOXY_YOLOG_OUTPUT_COUNT; ii++) {
...
static void
epoxy_yolog_get_formats(struct epoxy_yolog_output_st *output,
                  int level,
                  struct epoxy_yolog_msginfo_st *colors)
{
    if (output->use_color) {
        ...
        case EPOXY_YOLOG_CRIT:
        case EPOXY_YOLOG_ERROR:
            colors->co_line = "\033[" _BRIGHT_FG ";" _FG _RED "m";
            break;

        case EPOXY_YOLOG_WARN:
            colors->co_line = "\033[" _FG _YELLOW "m";
            break;

        case EPOXY_YOLOG_DEBUG:
        case EPOXY_YOLOG_TRACE:
        case EPOXY_YOLOG_STATE:
        case EPOXY_YOLOG_RANT:
...
epoxy_yolog_vlogger(epoxy_yolog_context *ctx,
              ...
              va_list ap)
{
    struct epoxy_yolog_msginfo_st msginfo;
    const char *prefix;
    int ii;
    struct epoxy_yolog_output_st *outputs[EPOXY_YOLOG_OUTPUT_COUNT];
    ...
    for (ii = 0; ii < EPOXY_YOLOG_OUTPUT_COUNT; ii++) {
        struct epoxy_yolog_output_st *out = outputs[ii];
        FILE *fp;
        ...
        epoxy_yolog_get_formats(out, level, &msginfo);

        epoxy_yolog_dest_lock(out);

#ifndef va_copy
#define EPOXY_YOLOG_VACOPY_OVERRIDE
#ifdef __GNUC__
...
#define va_copy(dst, src) (dst) = (src)
        ...
        epoxy_yolog_fmt_write(fmtv, fp, &msginfo);
        {
            va_list vacp;
            va_copy(vacp, ap);
        ...
        fprintf(fp, "%s\n", msginfo.co_reset);
        fflush(fp);
        epoxy_yolog_dest_unlock(out);
    }
#ifdef EPOXY_YOLOG_VACOPY_OVERRIDE
#undef va_copy
#undef EPOXY_YOLOG_VACOPY_OVERRIDE
#endif
}
             ...
             ...)
{
    va_list ap;
    va_start(ap, fmt);
    epoxy_yolog_vlogger(ctx, level, file, line, fn, fmt, ap);
    va_end(ap);
}
...
epoxy_yolog_implicit_logger(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    epoxy_yolog_vlogger(Yolog_Implicit.ctx,
                        ...
                        const char *format)
{
    ...
    epoxy_yolog_set_fmtstr(&grp->o_screen, format, 1);
}
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
int syscall(int, ...);
#endif
#define epoxy_yolog_fprintf_thread(f) fprintf(f, "%d", syscall(SYS_gettid))

#else /* other POSIX non-linux systems */
static void
epoxy_yolog_fprintf_thread(FILE *f) {
...
#define epoxy_yolog_get_pid getpid

#else
#define epoxy_yolog_fprintf_thread(f)
#define epoxy_yolog_get_pid() -1
...
static
const char *
epoxy_yolog_strlevel(epoxy_yolog_level_t level)
{
    if (!level) {
        return "";
    }
...
epoxy_yolog_fmt_compile(const char *fmtstr)
{
    const char *fmtp = fmtstr;
    struct epoxy_yolog_fmt_st *fmtroot, *fmtcur;
    ...
    fmtcur->type = EPOXY_YOLOG_FMT_USTRING;
    ...
    #define _cmpopt(s) (strncmp(optbuf, s, \
        ...
        if (_cmpopt("ep")) {
            /* epoch */
            fmtcur->type = EPOXY_YOLOG_FMT_EPOCH;

        } else if (_cmpopt("pi")) {
            /* pid */
            fmtcur->type = EPOXY_YOLOG_FMT_PID;

        } else if (_cmpopt("ti")) {
            /* tid */
            fmtcur->type = EPOXY_YOLOG_FMT_TID;

        } else if (_cmpopt("le")) {
            /* level */
            fmtcur->type = EPOXY_YOLOG_FMT_LVL;

        } else if (_cmpopt("pr")) {
            /* prefix */
            fmtcur->type = EPOXY_YOLOG_FMT_TITLE;

        } else if (_cmpopt("fi")) {
            /* filename */
            fmtcur->type = EPOXY_YOLOG_FMT_FILENAME;

        } else if (_cmpopt("li")) {
            /* line */
            fmtcur->type = EPOXY_YOLOG_FMT_LINE;

        } else if (_cmpopt("fu")) {
            /* function */
            fmtcur->type = EPOXY_YOLOG_FMT_FUNC;

        } else if (_cmpopt("co")) {
            /* color */
            fmtcur->type = EPOXY_YOLOG_FMT_COLOR;
        } else {
            goto GT_ERROR;
        }
    ...
    fmtroot[n_used+1].type = EPOXY_YOLOG_FMT_LISTEND;
    return fmtroot;
...
{
    struct epoxy_yolog_fmt_st *fmtcur = fmts;

    while (fmtcur->type != EPOXY_YOLOG_FMT_LISTEND) {
        switch (fmtcur->type) {
        case EPOXY_YOLOG_FMT_USTRING:
            fprintf(fp, "%s", fmtcur->ustr);
            goto GT_NOUSTR;
            break;

        case EPOXY_YOLOG_FMT_EPOCH:
            fprintf(fp, "%d", (int)time(NULL));
            break;

        case EPOXY_YOLOG_FMT_PID:
            fprintf(fp, "%d", epoxy_yolog_get_pid());
            break;

        case EPOXY_YOLOG_FMT_TID:
            epoxy_yolog_fprintf_thread(fp);
            break;

        case EPOXY_YOLOG_FMT_LVL:
            fprintf(fp, "%s", epoxy_yolog_strlevel(minfo->m_level));
            break;

        case EPOXY_YOLOG_FMT_TITLE:
            fprintf(fp, "%s%s%s",
                    minfo->co_title, minfo->m_prefix, minfo->co_reset);
            break;

        case EPOXY_YOLOG_FMT_FILENAME:
            fprintf(fp, "%s", minfo->m_file);
            break;

        case EPOXY_YOLOG_FMT_LINE:
            fprintf(fp, "%d", minfo->m_line);
            break;

        case EPOXY_YOLOG_FMT_FUNC:
            fprintf(fp, "%s", minfo->m_func);
            break;

        case EPOXY_YOLOG_FMT_COLOR:
...
epoxy_yolog_set_fmtstr(struct epoxy_yolog_output_st *output,
                 ...
                 int replace)
{
    struct epoxy_yolog_fmt_st *newfmt;

    if (output->fmtv && replace == 0) {
        return -1;
    }

    newfmt = epoxy_yolog_fmt_compile(fmt);
    if (!newfmt) {
        return -1;
    }
...
#ifdef EPOXY_YOLOG_APESQ_STATIC
...
#endif /* EPOXY_YOLOG_APESQ_STATIC */

#else
#endif /* GENYL_APESQ_INLINED */

static epoxy_yolog_context *
epoxy_yolog_context_by_name(epoxy_yolog_context *contexts,
                      int ncontexts,
                      const char *name)
{
    int ii = 0;
...
static int
epoxy_yolog_level_by_name(const char *lvlstr)
{
                ...
                const char *envstr)
{
    ...
    if (!grp) {
        grp = epoxy_yolog_get_global()->parent;
    }
    ...
    do {
        ...
        *level = '\0';
        ...
        ctx = epoxy_yolog_context_by_name(grp->contexts, grp->ncontexts, subsys);
        ...
        minlvl = epoxy_yolog_level_by_name(level);
        if (minlvl == -1) {
            fprintf(stderr, "Yolog: Bad level specified '%s'\n", level);
            goto GT_NEXT;
        }

        ctx->olevels[EPOXY_YOLOG_OUTPUT_SCREEN] = minlvl;
        epoxy_yolog_sync_levels(ctx);
...
get_minlevel(struct apesq_entry_st *secent)
{
    ...
    level = epoxy_yolog_level_by_name(value->strdata);
    if (level == -1) {
...
handle_subsys_output(
        epoxy_yolog_context *ctx,
        struct apesq_entry_st *ent,
        char *logroot,
        struct epoxy_yolog_fmt_st *fmtdef,
        int *fmtdef_used)
{
    int minlevel = -1;
    ...
    for (current = oents; *current; current++) {
        ...
        for (; *onames; onames++) {
            ...
            if (strcmp(*onames, "$screen$") == 0) {
                olix = EPOXY_YOLOG_OUTPUT_SCREEN;

            } else if (strcmp(*onames, "$globalfile$") == 0) {
                olix = EPOXY_YOLOG_OUTPUT_GFILE;

            } else {
                FILE *fp;
                char *fname;
                olix = EPOXY_YOLOG_OUTPUT_PFILE;
                ...
                if ( (apval = apesq_get_values(osec, "Format"))) {
                    ctx->o_alt->fmtv = epoxy_yolog_fmt_compile(apval->strdata);
            ...
            if (olix == EPOXY_YOLOG_OUTPUT_PFILE) {
                 ...
                 const char *filename)
{
    ...
    struct epoxy_yolog_fmt_st *fmtdfl = NULL;
    int fmtdfl_used = 0;
    ...
    if ((apval = apesq_get_values(secroot, "Format"))) {
        fmtdfl = epoxy_yolog_fmt_compile(apval->strdata);
    }
    ...
    for (cursecent = secents; *cursecent && gout_count < 2; cursecent++) {
        ...
        struct epoxy_yolog_output_st *out;
        ...
        if ( (apval = apesq_get_values(sec, "Format"))) {
            ...
            out->fmtv = epoxy_yolog_fmt_compile(apval->strdata);
        } else if (fmtdfl) {
            out->fmtv = fmtdfl;
            fmtdfl_used = 1;
        } else {
            ...
            out->fmtv = epoxy_yolog_fmt_compile(EPOXY_YOLOG_FORMAT_DEFAULT);
        }
    ...
    for (cursecent = secents; *cursecent; cursecent++) {
        ...
        for (; *secnames; secnames++) {
            ...
            struct epoxy_yolog_context *ctx =
                    epoxy_yolog_context_by_name(grp->contexts,
                                          grp->ncontexts,
                                          *secnames);
            if (!ctx) {

                if (grp != epoxy_yolog_get_global()->parent) {
            ...
            if (tmplevel != -1) {
                int ii;
                for (ii = 0; ii < EPOXY_YOLOG_OUTPUT_COUNT; ii++) {
                    if (ctx->olevels[ii] == EPOXY_YOLOG_LEVEL_UNSET) {
                        ctx->olevels[ii] = tmplevel;
                    }
                }
            }

            ctx->level = EPOXY_YOLOG_LEVEL_UNSET;
            epoxy_yolog_sync_levels(ctx);
        }
    }
    free (secents);
...
static epoxy_yolog_context epoxy_yolog_logging_contexts_real[
    EPOXY_YOLOG_LOGGING_SUBSYS__COUNT
] = { { 0 } };

epoxy_yolog_context* epoxy_yolog_logging_contexts = epoxy_yolog_logging_contexts_real;
epoxy_yolog_context_group epoxy_yolog_log_group;

#include <string.h> /* for memset */
#include <stdlib.h> /* for getenv */
void
epoxy_yolog_init(const char *filename)
{
    epoxy_yolog_context* ctx;
    memset(epoxy_yolog_logging_contexts, 0, sizeof(epoxy_yolog_context) * EPOXY_YOLOG_LOGGING_SUBSYS__COUNT);

   ctx = epoxy_yolog_logging_contexts + EPOXY_YOLOG_LOGGING_SUBSYS_CLIENT;
   ctx->prefix = "client";

   ctx = epoxy_yolog_logging_contexts + EPOXY_YOLOG_LOGGING_SUBSYS_ACCEPT;
   ctx->prefix = "accept";

   ctx = epoxy_yolog_logging_contexts + EPOXY_YOLOG_LOGGING_SUBSYS_LCBT;
   ctx->prefix = "lcbt";
   ...
   memset(&epoxy_yolog_log_group, 0, sizeof(epoxy_yolog_log_group));
   epoxy_yolog_log_group.ncontexts = EPOXY_YOLOG_LOGGING_SUBSYS__COUNT;
   epoxy_yolog_log_group.contexts = epoxy_yolog_logging_contexts;

   epoxy_yolog_init_defaults(
        &epoxy_yolog_log_group,
        EPOXY_YOLOG_DEFAULT,
        "EPOXY_DEBUG_COLOR",
        "EPOXY_DEBUG_LEVEL"
    );
    ...
    if (filename) {
        epoxy_yolog_parse_file(&epoxy_yolog_log_group, filename);
        /* if we're a static build, also set the default levels */

#ifdef GENYL_YL_STATIC
        epoxy_yolog_parse_file(NULL, filename);
#endif

    }

    if (getenv("EPOXY_DEBUG_PREFS")) {
        epoxy_yolog_parse_envstr(&epoxy_yolog_log_group, getenv("EPOXY_DEBUG_PREFS"));
    }
}
